...
class SentimentVoteClassifier(ClassifierI):

    def __init__(self):
        self._classifiers = []

        file = open(config.__project_dir__ + "data/nlp_engine/sentiment-models/nb", "rb")
        naive_bayes = pickle.load(file)
        self._classifiers.append(naive_bayes)
        file.close

        file = open(config.__project_dir__ + "data/nlp_engine/sentiment-models/multinomial_nb", "rb")
        multinomial_nb = pickle.load(file)
        self._classifiers.append(multinomial_nb)
        file.close()

        file = open(config.__project_dir__ + "data/nlp_engine/sentiment-models/bernoulli_nb", "rb")
        bernoulli_ng = pickle.load(file)
        self._classifiers.append(bernoulli_ng)
        file.close()

        file = open(config.__project_dir__ + "data/nlp_engine/sentiment-models/logistic_regression", "rb")
        logistic_regression = pickle.load(file)
        self._classifiers.append(logistic_regression)
        file.close()

        file = open(config.__project_dir__ + "data/nlp_engine/sentiment-models/sgdc", "rb")
        sgdc = pickle.load(file)
        self._classifiers.append(sgdc)
        file.close()
    ...
    def find_features(self, doc_words):
        file = open(config.__project_dir__ + "data/nlp_engine/sentiment-models/features", "rb")
        word_features = pickle.load(file)
        file.close()
